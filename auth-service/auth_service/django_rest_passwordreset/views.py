...
class ResetPasswordRequestToken(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not active_user_found and not getattr(settings, "DJANGO_REST_PASSWORDRESET_NO_INFORMATION_LEAKAGE", False):
            logger.warning(
                "Reset password We couldn't find an account associated with that email address {}.".format(email)
            )
            return Response({"status": "OK"})
        ...
        for user in users:
            if user.eligible_for_reset() and _unicode_ci_compare(
                ...
                if user.last_password_reset:
                    reset_timeout = (datetime.now(timezone.utc) - user.last_password_reset).seconds
                    if not reset_timeout > settings.DJANGO_REST_PASSWORDRESET_TIMEOUT:
                        return Response({"message": "Password reset timeout"}, status=403)

                # send password reset token
                password_reset_token_created(token)
